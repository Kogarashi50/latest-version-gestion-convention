...
const ProjetForm = ({
    ...
    const validateForm = () => {
        ...
        const checkNumeric = (field, name) => { const v = formData[field]; if (v === '' || v === null || isNaN(parseCurrency(v)) || parseCurrency(v) < 0) errors[field] = `${name} requis/numérique positif.`; };
        checkNumeric('Cout_CRO', 'Coût CRO');
        checkNumeric('Cout_Projet', 'Coût Projet');
        const checkPercent = (field, name) => { const v = formData[field]; if (v === '' || v === null || isNaN(parseCurrency(v)) || parseCurrency(v) < 0 || parseCurrency(v) > 100) errors[field] = `${name} requis (0-100).`; };
                ...
                <div className="flex-grow-1">
                    ...
                    <Form noValidate onSubmit={handleSubmit}>
                        ...
                        <Row className="mb-1 g-3">
                         ...
                         <Row className="mb-1 g-3">
                              <Form.Group as={Col} md={4} controlId="formEtatAvanPhysi"><Form.Label className="small mb-1 fw-medium">Av. Physi (%)<span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.Etat_Avan_Physi} required type="number" name="Etat_Avan_Physi" value={formData.Etat_Avan_Physi} onChange={handleChange} size="sm" step="0.01" min="0" max="100"/><Form.Control.Feedback type="invalid">{formErrors.Etat_Avan_Physi}</Form.Control.Feedback></Form.Group>
                              <Form.Group as={Col} md={4} controlId="formCoutProjet"><Form.Label className="small mb-1 fw-medium">Coût Projet (MAD)<span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.Cout_Projet} required type="number" name="Cout_Projet" value={formData.Cout_Projet} onChange={handleChange} size="sm" step="0.01" min="0"/><Form.Control.Feedback type="invalid">{formErrors.Cout_Projet}</Form.Control.Feedback></Form.Group>
                              <Form.Group as={Col} md={4} controlId="formCoutCRO"><Form.Label className="small mb-1 fw-medium">Coût Part CRO (MAD)<span className="text-danger">*</span></Form.Label><Form.Control className={FORM_CONTROL_CLASS} isInvalid={!!formErrors.Cout_CRO} required type="number" name="Cout_CRO" value={formData.Cout_CRO} onChange={handleChange} size="sm" step="0.01" min="0"/><Form.Control.Feedback type="invalid">{formErrors.Cout_CRO}</Form.Control.Feedback></Form.Group>
                         </Row>
